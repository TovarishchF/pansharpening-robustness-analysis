...
class SFIMPansharpening:
    """
    Паншарпинг методом Smoothing Filter-based Intensity Modulation (SFIM)
    """
    ...
    def _create_mean_filtered_pan(self, pan_data: np.ndarray) -> np.ndarray:
        """
        Создание сглаженной версии PAN изображения
        с использованием фильтра скользящего среднего
        """
        # Применяем фильтр скользящего среднего
        pan_smooth = uniform_filter(pan_data, size=self.window_size, mode='reflect')

        logger.debug(f"Сглаженный PAN: min={np.min(pan_smooth):.4f}, max={np.max(pan_smooth):.4f}")

        return pan_smooth

    def _apply_sfim_pansharpening(self, ms_upsampled: np.ndarray, pan_data: np.ndarray) -> np.ndarray:
        """
        Применение SFIM паншарпенинга
        SFIM = MS * (PAN / PAN_smooth)
        """
        # Создаем сглаженную версию PAN
        pan_smooth = self._create_mean_filtered_pan(pan_data)

        # Избегаем деления на ноль
        pan_smooth = np.where(pan_smooth == 0, 1e-8, pan_smooth)

        # Вычисляем коэффициент модуляции
        modulation_ratio = pan_data / pan_smooth

        # Применяем SFIM к каждому MS каналу
        pansharpened = np.zeros_like(ms_upsampled, dtype=np.float32)
        for i in range(ms_upsampled.shape[0]):
            pansharpened[i] = ms_upsampled[i] * modulation_ratio

            # Ограничиваем значения разумными пределами
            pansharpened[i] = np.clip(pansharpened[i], 0, np.percentile(pansharpened[i], 99.9))

        return pansharpened

    def process_single_polygon(self, ms_path: Path, pan_path: Path, biome_name: str, fid: int) -> Optional[
        PansharpeningResult]:
        """Обработка одного полигона методом SFIM"""
        ...
        try:
            ...
            # Применяем метод SFIM
            pansharpened_data = self._apply_sfim_pansharpening(ms_upsampled, pan_data)
...
def main():
    ...
    logger.info("Запуск паншарпенинга методом SFIM")

    try:
        sfim_processor = SFIMPansharpening()
        sfim_processor.process_all_biomes()
        # logger.info("Паншарпинг SFIM успешно завершен")
