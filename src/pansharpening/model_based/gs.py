...
class GramSchmidtPansharpening:
    """
    Паншарпинг методом Gram-Schmidt (GS)
    Базовый метод без адаптивных коэффициентов
    """
    ...
    def __init__(self, config_path: str = None):
        ...
        self.method_name = "gram_schmidt"
        self._setup_paths()
        logger.info(f"Инициализирован метод Gram-Schmidt паншарпенинга")
    ...
    def _calculate_intensity(self, ms_upsampled: np.ndarray) -> np.ndarray:
        """
        Вычисление интенсивности как среднего значения MS каналов
        В базовом методе Gram-Schmidt используется простое среднее
        """
    ...
    def _apply_gram_schmidt_transform(self, ms_upsampled: np.ndarray, pan_matched: np.ndarray,
                                    intensity: np.ndarray) -> np.ndarray:
        """
        Применение базового преобразования Gram-Schmidt для паншарпенинга
        Formula: PS_i = MS_upsampled_i + (PAN_matched - intensity)
        В базовом методе используется единый коэффициент инъекции = 1 для всех каналов
        """
        ...
        # Применяем инъекцию деталей ко всем каналам с единичным коэффициентом
        pansharpened = np.zeros_like(ms_upsampled, dtype=np.float32)
        for i in range(ms_upsampled.shape[0]):
            pansharpened[i] = ms_upsampled[i] + detail_component
    ...
    def process_single_polygon(self, ms_path: Path, pan_path: Path, biome_name: str, fid: int) -> Optional[
        PansharpeningResult]:
        """
        Обработка одного полигона методом Gram-Schmidt паншарпенинга
        """
        ...
        try:
            ...
            # Применяем метод Gram-Schmidt паншарпенинга
            pansharpened_data = self._apply_gram_schmidt_transform(ms_upsampled, pan_matched, intensity)
...
def main():
    ...
    logger.info("Запуск паншарпенинга методом Gram-Schmidt")

    try:
        gs_processor = GramSchmidtPansharpening()
        gs_processor.process_all_biomes()
        logger.info("Паншарпинг Gram-Schmidt успешно завершен")
